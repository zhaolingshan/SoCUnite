...
class Upvoteforums extends StatefulWidget { //managed for comments
final Post post;
final Comments comment; //takes in a comment 
 Upvoteforums({Key key, @required this.comment, @required this.post}) : super(key: key);
  @override
  _UpvoteforumsState createState() => _UpvoteforumsState();
}
        ...
        IconButton(
            onPressed: () async {
              ...
              await Firestore.instance.collection('public').document('CS2030')
                 .collection('Forums').document(widget.post.documentid).collection('comments')
