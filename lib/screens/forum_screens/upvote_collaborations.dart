...
class Upvotecollaborations extends StatefulWidget { //managed for comments
final Comments comment; //takes in a comment 
final Collaboration post;
 Upvotecollaborations({Key key, @required this.comment, @required this.post}) : super(key: key);
  @override
  _UpvotecollaborationsState createState() => _UpvotecollaborationsState();
}
        ...
        IconButton(
            onPressed: () async {
              ...
              await Firestore.instance.collection('public').document('collaborations')
                 .collection('Collaborations').document(widget.post.documentid).collection('comments')
