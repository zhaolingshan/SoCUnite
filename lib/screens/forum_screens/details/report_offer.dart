...
class ReportOffers extends StatefulWidget {
  final Offer post; //forum

  ReportOffers({Key key, @required this.post}) : super(key: key);
  @override
    ...
    return Scaffold(
      ...
      body: Column(
        children: <Widget>[
          ...
          child:
          Text(
            "Why are you reporting this post under 'Internship Offers'?",
          ...
          ListTile(
              ...
              final uid = await Provider.of(context).auth.getCurrentUID();
                ...
                await Firestore.instance.collection('users').document(widget.post.ownerid).collection('private_offers')
                .document(widget.post.documentid).setData({
                  'reported': widget.post.reported,
                },merge : true).then((_){
                print("success!");
                ...
                await Firestore.instance.collection('users').getDocuments().then((querySnapshot) {
                  querySnapshot.documents.forEach((element) { //element is each uid 
                    ...
                    .collection('saved_offers').getDocuments().then((querySnapshot) { //result is each saved post
                      querySnapshot.documents.forEach((result) {
                        Firestore.instance.collection('users').document(element.documentID)
                      .collection('saved_offers').document(result.documentID).setData({
                        'reported': widget.post.reported, 
                ...
                DocumentSnapshot doc = await Firestore.instance.collection('public').document('internship_offers').
                collection('Offers').document(widget.post.documentid).get();
                    ...
                    .collection('saved_offers').getDocuments().then((querySnapshot) { //result is each saved post
                      querySnapshot.documents.forEach((result) {
                        if(widget.post.documentid == result.documentID) {
                          Firestore.instance.collection('users').document(element.documentID)
                      .collection('saved_offers').document(result.documentID).delete();
                        }                        
                      });                   
                    });
                  }); 
