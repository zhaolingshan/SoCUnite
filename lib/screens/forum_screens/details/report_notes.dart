...
class ReportNotes extends StatefulWidget {
  final PostNotes post; //forum

  ReportNotes({Key key, @required this.post}) : super(key: key);
  @override
  _ReportNotesState createState() => _ReportNotesState();
}

class _ReportNotesState extends State<ReportNotes> {
  @override
    ...
    return Scaffold(
      ...
      body: Column(
        children: <Widget>[
          ...
          child:
          Text(
            "Why are you reporting this post under 'Notes'?",
          ...
          ListTile(
              ...
              final uid = await Provider.of(context).auth.getCurrentUID();
                ...
                await Firestore.instance.collection('public').document('CS2030').collection('Notes')
                ...
                await Firestore.instance.collection('users').document(widget.post.ownerid).collection('private_notes')
                .document(widget.post.documentid).setData({
                  'reported': widget.post.reported,
                },merge : true).then((_){
                print("success!");
                ...
                await Firestore.instance.collection('users').getDocuments().then((querySnapshot) {
                  querySnapshot.documents.forEach((element) { //element is each uid 
                    ...
                    .collection('saved_notes').getDocuments().then((querySnapshot) { //result is each saved post
                      querySnapshot.documents.forEach((result) {
                        Firestore.instance.collection('users').document(element.documentID)
                      .collection('saved_notes').document(result.documentID).setData({
                        'reported': widget.post.reported, 
                ...
                collection('Notes').document(widget.post.documentid).get();
                    ...
                    .collection('saved_notes').getDocuments().then((querySnapshot) { //result is each saved post
                      querySnapshot.documents.forEach((result) {
                        if(widget.post.documentid == result.documentID) {
                          Firestore.instance.collection('users').document(element.documentID)
                      .collection('saved_notes').document(result.documentID).delete();
                        }                        
                      });                   
                    });
                  }); 
