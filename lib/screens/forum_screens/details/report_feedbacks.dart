...
class ReportFeedbacks extends StatefulWidget {
  final PostFeedback post; //forum

  ReportFeedbacks({Key key, @required this.post}) : super(key: key);
  @override
  _ReportFeedbacksState createState() => _ReportFeedbacksState();
}

class _ReportFeedbacksState extends State<ReportFeedbacks> {
  @override
    ...
    return Scaffold(
      ...
      body: Column(
        children: <Widget>[
          ...
          child:
          Text(
            "Why are you reporting this post under 'Feedbacks'?",
          ...
          ListTile(
              ...
              final uid = await Provider.of(context).auth.getCurrentUID();
                ...
                await Firestore.instance.collection('public').document('CS2030').collection('Feedbacks')
                ...
                await Firestore.instance.collection('users').document(widget.post.ownerid).collection('private_feedbacks')
                .document(widget.post.documentid).setData({
                  'reported': widget.post.reported,
                },merge : true).then((_){
                print("success!");
                ...
                await Firestore.instance.collection('users').getDocuments().then((querySnapshot) {
                  querySnapshot.documents.forEach((element) { //element is each uid 
                    ...
                    .collection('saved_feedbacks').getDocuments().then((querySnapshot) { //result is each saved post
                      querySnapshot.documents.forEach((result) {
                        Firestore.instance.collection('users').document(element.documentID)
                      .collection('saved_feedbacks').document(result.documentID).setData({
                        'reported': widget.post.reported, 
                ...
                collection('Feedbacks').document(widget.post.documentid).get();
                    ...
                    .collection('saved_feedbacks').getDocuments().then((querySnapshot) { //result is each saved post
                      querySnapshot.documents.forEach((result) {
                        if(widget.post.documentid == result.documentID) {
                          Firestore.instance.collection('users').document(element.documentID)
                      .collection('saved_feedbacks').document(result.documentID).delete();
                        }                        
                      });                   
                    });
                  }); 
