...
class ReportExperiences extends StatefulWidget {
  final Experience post; //forum

  ReportExperiences({Key key, @required this.post}) : super(key: key);
  @override
  _ReportExperiencesState createState() => _ReportExperiencesState();
}

class _ReportExperiencesState extends State<ReportExperiences> {
  @override
    ...
    return Scaffold(
      ...
      body: Column(
        children: <Widget>[
          ...
          child:
          Text(
            "Why are you reporting this post under 'Internship Experiences'?",
          ...
          ListTile(
              ...
              final uid = await Provider.of(context).auth.getCurrentUID();
                ...
                await Firestore.instance.collection('users').document(widget.post.ownerid).collection('private_experiences')
                .document(widget.post.documentid).setData({
                  'reported': widget.post.reported,
                },merge : true).then((_){
                print("success!");
                ...
                await Firestore.instance.collection('users').getDocuments().then((querySnapshot) {
                  querySnapshot.documents.forEach((element) { //element is each uid 
                    ...
                    .collection('saved_experiences').getDocuments().then((querySnapshot) { //result is each saved post
                      querySnapshot.documents.forEach((result) {
                        Firestore.instance.collection('users').document(element.documentID)
                      .collection('saved_experiences').document(result.documentID).setData({
                        'reported': widget.post.reported, 
                ...
                DocumentSnapshot doc = await Firestore.instance.collection('public').document('internship_experiences').
                collection('Experiences').document(widget.post.documentid).get();
                    ...
                    .collection('saved_experiences').getDocuments().then((querySnapshot) { //result is each saved post
                      querySnapshot.documents.forEach((result) {
                        if(widget.post.documentid == result.documentID) {
                          Firestore.instance.collection('users').document(element.documentID)
                      .collection('saved_experiences').document(result.documentID).delete();
                        }                        
                      });                   
                    });
                  }); 
